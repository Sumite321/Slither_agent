...
public class GuiGame extends Gui {
    ...
    public void render(float mouseX, float mouseY) {
        ...
        if (!loading) {
            ...
            String rank="Rank " + client.rank + "/" + client.snakeCount;
            rank+=" snake count="+client.getSnakes().size();
            rank+=" ("+client.player.posX+", "+client.player.posY+")";
            drawString(rank, 3.0F, renderResolution.getHeight() - 18.0F, 0.5F, 0xAAAAAA);
    ...
    public void keyPressed(int key, char character) {
        ...
        if (key == Keyboard.KEY_A){
            ...
            client.player.accelerating=true;
        }
    ...
    private void saveGame(SlytherClient client) {
        File f = new File("./saved.game");
        for (Snake<?> snake:client.getSnakes()){
            if (snake.equals(client.player)){
                snake.isPlayer=true;
            }
        }
        ArrayList<Snake<?>> snakes=new ArrayList<>(client.getSnakes());
        try {
            ObjectOutputStream fos = new ObjectOutputStream(new FileOutputStream(f));
            
            fos.writeObject(snakes);
            fos.flush();
            fos.close();
